...
def plot(a, b, t1, t2, labelsA=None, labelsB=None, sort=True, figsize=(8, 8),
    ...
    Z1 = sclust.hierarchy.dendrogram(link1, orientation='left', labels=labelsA, color_threshold=t1, **dend_kwargs) # max(link1[:,2])+1

    # Compute and plot right dendrogram.
    ax2 = fig.add_axes([0.7, 0.1, 0.25, 0.8])  # [0.3, 0.71, 0.6, 0.2])
    Z2 = sclust.hierarchy.dendrogram(link2, labels=labelsB, orientation='right', color_threshold=t2, **dend_kwargs) # max(link2[:,2])+1
