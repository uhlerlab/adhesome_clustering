...
def large_average_submatrix_adj(data, chr1, chr2, threshold_new, max_submatrix_size, pair):
    # store some data on iterations
    dir = config["CURRENT_LAS_OUT_DIR"] + str(chr1) + '_' + str(chr2) + '/'
    ...
    while (continue_search):
        rows, cols, sub_matrix, best_score = search_main(data, dir, iter, max_submatrix_size, pair)
...
def search_main(data, dir, iter, max_submatrix_size, pair):
    # keep track of submatrix params that you get out
    chrom = str(pair[1])
    num_iter = len(config['loci_dict'][chrom])
    search_attributes = np.empty((num_iter, 6))
    for iteration, locus in enumerate(config['loci_dict'][chrom]):
        start_row, k, start_col, l, curr_score = search(data, max_submatrix_size, locus)
        search_attributes[iteration] = start_row, k, start_col, l ,curr_score, locus

    # save the iterations
    np.savetxt(dir + 'sub_matrix' + str(iter) + '.txt', search_attributes)


    best_start_row, best_k, best_start_col, best_l, best_score, locus =  search_attributes[np.argmax(search_attributes[:,4])]
...
def search(data, max_submatrix_size, locus):
    # run search procedure with fixed k, l first
    max_num_rows = int(max_submatrix_size/config["HIC_RESOLN"])
    max_num_cols = int(max_submatrix_size/config["HIC_RESOLN"])
    k = random.randint(1, max_num_rows)
    l = random.randint(1, max_num_cols)
    row_set, col_set = search_fixed_k_l(data, k, l, locus)
...
def search_fixed_k_l(data, k, l, locus):
    # initialize (select l adjacent columns at random)
    num_rows = data.shape[0]
    num_cols = data.shape[1]
    # use input locus as starting position for column
    start_col = locus
    col_set = data[:,start_col:start_col+l]
...
def numclust_avg(pair):
    chr1, chr2 = pair
    fname = config["CURRENT_LAS_OUT_DIR"]  + 'intermingling_regions.chr' + str(chr1) + '_chr' + str(chr2) + '.avg_filt.csv'
    ...
    if (os.path.isfile(fname) == True):
        ...
        plt.figure()
        plt.plot(list(range(df_intermingling.shape[0])), df_intermingling['score'], 'o-')
        plt.xlabel('Cluster #')
        plt.ylabel('Score')
        plt.savefig(config["CURRENT_LAS_OUT_DIR"] + 'cluster_score.chr' + str(chr1) + '_chr' + str(chr2) + '.png')
        plt.close()

        plt.figure()
        plt.plot(list(range(df_intermingling.shape[0])), df_intermingling['avg'], 'o-')
        plt.xlabel('Cluster #')
        plt.ylabel('Average')
        plt.savefig(config["CURRENT_LAS_OUT_DIR"] + 'cluster_average.chr' + str(chr1) + '_chr' + str(chr2) + '.png')
        plt.close()
...
def determine_min_max_hic():
    ...
    for pair in chr_pairs:
        ...
        if (os.path.isfile(fname) == True):
                # read in hic matrix
                hic_filename =  config["CURRENT_HIC_FILT_DIR"] +'hic_chr' + str(chr1) + '_chr' + str(chr2) + '_zscore.txt'
...
def draw_identified_LASregions(pair, minl, maxl):
    ...
    if (os.path.isfile(fname) == True):
        ...
        #plt.savefig(config["CURRENT_LAS_OUT_DIR"] + 'hic_transformed_rescaled.chr' + str(chr1) + '_chr' + str(chr2) + '.png')
        plt.savefig(config["CURRENT_LAS_OUT_DIR"] + 'hic_transformed_rescaled.chr' + str(chr1) + '_chr' + str(chr2) + 'commonscale.png')
...
def run_LAS(pair, threshold_new, max_submatrix_size, iters):
    chr1, chr2 = pair
    fname = config["CURRENT_LAS_OUT_DIR"] + 'intermingling_regions.chr' + str(chr1) + '_chr' + str(chr2) +'.avg_filt.csv'
    if (os.path.isfile(fname) == False):
        ...
        # read in hic matrix
        hic_filename =  config["CURRENT_HIC_FILT_DIR"] +'hic_chr' + str(chr1) + '_chr' + str(chr2) + '_zscore.txt'
        df = pd.read_csv(hic_filename, header=0, index_col=0)
        data = df.values

        # run LAS algorithm
        start_rows, stop_rows, start_cols, stop_cols, best_score_list, avg_list = large_average_submatrix_adj(data, chr1, chr2, threshold_new, max_submatrix_size, pair)
...
def main():
    ...
    iters = config['iter']
    max_submatrix_size = config['max_submatrix_size']
    ...
    print('Run LAS')
    config['CURRENT_LAS_OUT_DIR'] = config["LAS_OUT_DIR"]
    config['CURRENT_HIC_FILT_DIR'] = config["HIC_FILT_DIR"]
    Parallel(n_jobs = config['NUM_PROC'], backend='loky')(delayed(run_LAS)(pair, threshold_new, max_submatrix_size, iters) for pair in chr_pairs)  

    print('Compute maximum and minimum HiC across whole genome')
    minl, maxl = determine_min_max_hic()
    
    print('Draw LAS regions')
    Parallel(n_jobs = config['NUM_PROC'])(delayed(draw_identified_LASregions)(pair, minl, maxl) for pair in chr_pairs)  
    
    print('Run numclust_avg')
