...
def build_loci_contact_list(dir_processed_hic, locus_list1, locus_list2):
    '''
    ...
    # Create empty list to record contacts between loci lists
    contacts_list = []
    
    # Loop over all interX Hi-C matrices and record contacts
    for pair in tqdm(chr_pairs):
        time.sleep(.01)
        chr1, chr2 = pair
        
        # Select loci
        loci_chr1_list1 = [int(locus.split('_')[3]) for locus in locus_list1 
                        if ('chr_'+str(chr1)+'_') in locus]
        loci_chr2_list1 = [int(locus.split('_')[3]) for locus in locus_list1 
                        if ('chr_'+str(chr2)+'_') in locus]
        loci_chr1_list2 = [int(locus.split('_')[3]) for locus in locus_list2 
                     if ('chr_'+str(chr1)+'_') in locus]
        loci_chr2_list2 = [int(locus.split('_')[3]) for locus in locus_list2 
                     if ('chr_'+str(chr2)+'_') in locus]
        ...
        # Fill in corresponding submatrix of gene_contacts_df by sum of Hi-C contacts across all gene loci
        flat = hic_chpair_df.loc[loci_chr1_list1,loci_chr2_list2].values.flatten().tolist()
        contacts_list.append(flat)
        flat = hic_chpair_df.loc[loci_chr1_list2,loci_chr2_list1].values.flatten().tolist()
        contacts_list.append(flat)

    contacts_list = list(itertools.chain.from_iterable(contacts_list))
