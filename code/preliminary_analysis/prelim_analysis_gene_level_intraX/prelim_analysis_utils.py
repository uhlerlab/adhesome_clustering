...
def get_adhesome_genes_loci(adhesome_loc_df, resol):
    ...
    adhesome_loc_df['loci'] = [np.arange(adhesome_loc_df.loc[i,'startLocus_id'], adhesome_loc_df.loc[i,'endLocus_id']+1) for i in range(ngenes)]
...
def build_adhesome_gene_contact_mat(chr0, adhesome_chr_loci, dir_processed_hic, resol):
    '''
    Builds a intrachromosomal gene x gene matrix where each entry (i,j) corresponds to the total Hi-C contacts between adhesome genes i and j (chosen from gene_list), obtained by summing up Hi-C contacts between the loci corresponding to genes i and j.
    Args:
    ...
    '''
    # List adhesome genes on chr0
    gene_list = np.array(adhesome_chr_loci[adhesome_chr_loci['chrom_int']==chr0].gene.drop_duplicates(keep='first'))
    
    # Create empty dataframe indexed by adhesome gene on chr0
    gene_contacts_df = pd.DataFrame(0, index=gene_list, columns=gene_list)

    # Load intraX HiC data for this chromosome
    processed_hic_filename = 'hic_'+'chr'+str(chr0)+'_'+'chr'+str(chr0)+'_norm1_filter3'+'.pkl'
    hic_chr_df = pickle.load(open(dir_processed_hic+processed_hic_filename, 'rb'))

    # Fill in corresponding submatrix of gene_contacts_df by sum of Hi-C contacts across all gene loci
    gene_pairs = itertools.combinations(gene_list,2)
    for gene1, gene2 in gene_pairs:
        gene1_loci = np.array(adhesome_chr_loci[adhesome_chr_loci['gene']==gene1]['loci']*resol)
        gene2_loci = np.array(adhesome_chr_loci[adhesome_chr_loci['gene']==gene2]['loci']*resol)
        gene_contacts_df.loc[gene1, gene2] = hic_chr_df.loc[gene1_loci,gene2_loci].sum().sum()
            
    return(gene_contacts_df)


def plot_heatmap(df, xticklabels, yticklabels, xlabel, ylabel, size, vmax, save_to='', fontsize=3):
    '''
    ...
    Args:
        ...
        fontsize: (int) fontsize for the labels
    Returns:
        void
    '''
