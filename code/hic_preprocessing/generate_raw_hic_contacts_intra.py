...
def get_raw_hic_sparse(raw_hic_dir, cell_type, resol_str, resol, quality, chr0):
    ...
    raw_hic_data['norm_locus_chr1'] = 1
    raw_hic_data['norm_locus_chr2'] = 1
...
def filter_outliers(df_transformed):
    ...
    col_orig = np.sum(df_diag0, axis = 0).as_matrix()
...
def main():
    ...
    for chr0 in chr_list:
        ...
        df_transformed = filter_outliers(df)
        
        # Plot and save to pickle HiC data after filtering out centromeres, repeats and outliers
        plotname = 'hic_'+'chr'+str(chr0)+'_'+'chr'+str(chr0)+'_raw_filter3'+'.eps'
        plot_dense_hic_dataframe(df_transformed, chr0, plotname, hic_plots_dir)
        picklename = processed_hic_data_dir+'hic_'+'chr'+str(chr0)+'_'+'chr'+str(chr0)+'_raw_filter3'+'.pkl'
