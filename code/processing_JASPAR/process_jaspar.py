...
def main():
    ...
    # Build fasta files for promoters of genes of interest
    print('Create fasta files for promoters of selected genes')
    adh_tf_genes = pickle.load(open(saving_dir+'adh_tf_genes.pkl', 'rb'))
    ucsc_to_hgnc = create_ucsc_hgnc_dict(dir_genome+'chrom_hg19.name')
    build_fasta_files(dir_genome+'prom_hg19.seq', ucsc_to_hgnc, adh_tf_genes, prom_hg19_seq_dir)
    
    # Run MOODS for each gene of interest
    print('Run MOODS on the promoters of all selected genes')
    MOODS_exec = "~/MOODS/python/scripts/moods-dna.py"
    all_prom_seq_files = os.listdir(prom_hg19_seq_dir)
    n_all_prom_seq_files = len(all_prom_seq_files)
    for i in range(n_all_prom_seq_files):
        gene_file = all_prom_seq_files[i]
        if gene_file.endswith(".fa"):
            ...
            result = subprocess.run(MOODS_exec+
                                    ...
                                    shell=True)
            print('Gene '+gene_name+' ('+ucsc_to_hgnc[gene_name]+') completed ('+str(i+1)+'/'+str(n_all_prom_seq_files)+')')
