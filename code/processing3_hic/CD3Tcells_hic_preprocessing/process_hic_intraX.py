...
def get_norm_hic_sparse(unprocessed_hic_dir, resol, chr1, chr2):
    '''
    Constructs dataframe of KR-normalized HiC data for a given chromosome pair in CD3 T cells.
    Args:
        unprocessed_hic_dir: (string) directory containing all normalized (unprocessed) hic data
        resol: (int) HiC resolution
    ...
    raw_mat_path = unprocessed_hic_dir+'chr'+str(chr1)+'_chr'+str(chr2)+'.txt'
    ...
    KRnorm_hic_data = pd.DataFrame(data={'locus_chr1':row_ind,'locus_chr2':col_ind,'value':contact_values})
    KRnorm_hic_data.columns = ['locus_chr1', 'locus_chr2', 'norm_value']

    return(KRnorm_hic_data)
...
def main():
    ...
    unprocessed_hic_dir = config['UNPROCESSED_HIC_DIR']
    processed_hic_dir = config['PROCESSED_HIC_DIR']
    ...
    # List all chromosomes and process the corresponding intrachromosomal HiC data
    ...
    for chr0 in chr_list:
        ...
        KRnorm_hic_data = get_norm_hic_sparse(unprocessed_hic_dir, resol, chr0, chr0)
        
        # Create dense HiC dataframe
        print('Create dense HiC dataframe')
        # Get chromosome size (in number of loci)
        chr0_size = int(df_sizes[df_sizes['chr']==str(chr0)]['size_loci'])
        # Construct normalized dense HiC dataframe
        df = get_dense_hic_dataframe(KRnorm_hic_data, chr0_size, chr0_size, resol)
        # Plot normalized dense HiC dataframe
        plotname = 'hic_'+'chr'+str(chr0)+'_'+'chr'+str(chr0)+'_norm1_filter0'+'.eps'
        plot_dense_hic_dataframe(df, chr0, chr0, plotname, hic_plots_dir)
        ...
        plot_dense_hic_dataframe(df_transformed, chr0, chr0, plotname, hic_plots_dir)
        picklename = processed_hic_dir+'hic_'+'chr'+str(chr0)+'_'+'chr'+str(chr0)+'_norm1_filter3'+'.pkl'
