...
def main():
    ...
    # Add Spearman correlation to interX and intraX edge lists
    ...
    for t in hic_threshold_list:
        wcc = cc.weighted_correlation_clustering_micha(selected_interX_edge_list, 
                                                       ...
                                                       num_calls=100)
        wcc_dict[t] = wcc[0]
    pickle.dump(wcc_dict, open(saving_dir+'dict_intra_wcc_withtfs.pickle','wb'))
    ...
    pickle.dump(ami_table, open(saving_dir+'ami_intra_wcc_withtfs.pickle','wb'))
    ...
    # Compute clusterings
    for t in hic_threshold_list:
        hc_dict[t] = cc.hierarchical_clustering(selected_interX_edge_list, 
                                                selected_intraX_edge_list,
                                                adh_tf_genes,
                                                t,
                                                with_intra,
                                                weights,
                                                hc_threshold)
    pickle.dump(hc_dict, open(saving_dir+'dict_intra_hc_withtfs.pickle','wb'))
    ...
    for t,u in itertools.combinations(hic_threshold_list,2):
        ami_table.loc[t,u] = adjusted_mutual_info_score(labels_true=hc_dict[t],labels_pred=hc_dict[u],
                                                        average_method='min')
        ami_table.loc[u,t] = ami_table.loc[t,u]
    pickle.dump(ami_table, open(saving_dir+'ami_intra_hc_withtfs.pickle','wb'))
    ...
    for t in hic_threshold_list:
        wcc = cc.weighted_correlation_clustering_micha(selected_interX_edge_list, 
                                                       ...
                                                       num_calls=100)
        wcc_dict[t] = wcc[0]
    pickle.dump(wcc_dict, open(saving_dir+'dict_nointra_wcc_withtfs.pickle','wb'))
    ...
    for t,u in itertools.combinations(hic_threshold_list,2):
        ami_table.loc[t,u] = adjusted_mutual_info_score(labels_true=wcc_dict[t],labels_pred=wcc_dict[u],
                                                        average_method='min')
        ami_table.loc[u,t] = ami_table.loc[t,u]
    pickle.dump(ami_table, open(saving_dir+'ami_nointra_wcc_withtfs.pickle','wb'))
    ...
    for t in hic_threshold_list:
        hc_dict[t] = cc.hierarchical_clustering(selected_interX_edge_list, 
                                                ...
                                                weights,
                                                hc_threshold)
    pickle.dump(hc_dict, open(saving_dir+'dict_nointra_hc_withtfs.pickle','wb'))
    ...
    for t,u in itertools.combinations(hic_threshold_list,2):
        ami_table.loc[t,u] = adjusted_mutual_info_score(labels_true=hc_dict[t],labels_pred=hc_dict[u],
                                                        average_method='min')
        ami_table.loc[u,t] = ami_table.loc[t,u]
    pickle.dump(ami_table, open(saving_dir+'ami_nointra_hc_withtfs.pickle','wb'))
