...
file1 = args[1] # Path to file for 1st partition (3-column BED format chromo-start-end; no header)
file2 = args[2] # Path to file for 2nd partition (3-column BED format chromo-start-end; no header)
chrSize_file = args[3] # Chromo size file [bp]
chrom = args[4] # Chromosome
OutFolder = args[5] # Folder where to save outputs
# nCpu = args[4] # Nbr of cpu available [use of foreach]
# fillInter = args[5] # Consider inter-TAD gaps as TADs ? [in our analyses: TRUE]
# meanWithInter = args[6] # Compute MoC for the inter-TADs separately and take the mean of TADs and interTADs MoCs [in our analyses: FALSE]
# correctClust = args[7] # Add penalty term for the number of domains ? [in our analyses: FALSE]
# binSize = args[8] # Bin size [bp]
# noMinusOne = args[9] # Do not substract 1 to the MoC ? [in our analyses: FALSE]
# fillInterGapZero = args[10] #Set score to 0 if comparing 1 TAD with interTAD ? [in our analyses: FALSE]
...
# Retrieve size of selected chromosome
chrSize_data <- read.csv(chrSize_file, sep="\t", header=FALSE)
chrSize <- chrSize_data[chrSize_data$V1 == paste0("chr", chrom), "V2"]

# Compute MoC                               
out = get_MoC(file1, file2, chrSize, nCpu = 1, fillInter = TRUE, meanWithInter = FALSE, correctClust = FALSE, binSize = NA, noMinusOne = FALSE, fillInterGapZero = FALSE)

# Construct filename
components1 <- unlist(strsplit(file1, "/"))
norm1 <- components1[9]
resol1 <- components1[10]
tad_list1 <- paste('tad', norm1, resol1, sep='_')                               
components2 <- unlist(strsplit(file2, "/"))
norm2 <- components2[9]
resol2 <- components2[10]
tad_list2 <- paste('tad', norm2, resol2, sep='_') 

# Add results to file
filename <- paste0('chr', chrom, '_moc.csv')
filepath <- file.path(OutFolder, filename)

if (!file.exists(filepath)) {
  file.create(filepath)
  file_conn <- file(filepath, "a")
  line <- paste('chrom', 'tad_list1', 'tad_list2', 'moc', sep='\t')
  writeLines(line, file_conn)
  close(file_conn)
}
file_conn <- file(filepath, "a")                                    
line <- paste(chrom, tad_list1, tad_list2, out, sep='\t')
writeLines(line, file_conn)
close(file_conn)