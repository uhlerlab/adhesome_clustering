...
def main():
    ...
    for pair in chr_pairs:
        ...
        KRnorm_hic_data = get_norm_hic_sparse(unprocessed_hic_dir, resol, chr1, chr2)
        ...
        df = get_dense_hic_dataframe(KRnorm_hic_data, chr1_size, chr2_size, resol)
    ...
    np.savetxt(processed_hic_dir + 'whole_genome_nonzero.logtrans.txt', nonzero_entries)
    mean, std = whole_genome_mean_std(nonzero_entries)
    z_score_hic_matrix(mean, std, chr_pairs, processed_hic_dir)
    
    # Output a file with locations that have been removed
    print('Output blacklisted loci file')
    output_blacklist_locations(chr_pairs, processed_hic_dir)
