...
def large_average_submatrix_adj(data, chr1, chr2, adhesome_loci_chr2, threshold_new):
    ...
    while (continue_search):
        rows, cols, sub_matrix, best_score = search_main(data, adhesome_loci_chr2, dir, iter)
...
def search(data, adhesome_loci_chr2):
    # run search procedure with fixed k, l first
    max_num_rows = int(10000000.0/config["HIC_RESOLUTION"])
    max_num_cols = int(10000000.0/config["HIC_RESOLUTION"])
    k = random.randint(1, max_num_rows)
    l = random.randint(1, max_num_cols)
    row_set, col_set = search_fixed_k_l(data, adhesome_loci_chr2, k, l)
...
def search_fixed_k_l(data, adhesome_loci_chr2, k, l):
...
def get_hic_matrix(hic_filename, chr1, chr2):
    ...
    row_ind = data[:,0]/config["HIC_RESOLUTION"]
    col_ind = data[:,1]/config["HIC_RESOLUTION"]
    contact_values = data[:,2]
    # obtain chromosome sizes
    chr1_size = get_chrom_size(chr1, config["GENOME_DIR"])/config["HIC_RESOLUTION"] + 1
    chr2_size = get_chrom_size(chr2, config["GENOME_DIR"])/config["HIC_RESOLUTION"] + 1

    hic_matrix = csr_matrix((contact_values, (row_ind, col_ind)), shape = (chr1_size, chr2_size))
    hic_dense = np.asarray(hic_matrix.todense())
    row_labels = np.arange(chr1_size)*config["HIC_RESOLUTION"]
    col_labels = np.arange(chr2_size)*config["HIC_RESOLUTION"]
...
def determine_min_max_hic():
    ...
    for pair in chr_pairs:
        ...
        if (os.path.isfile(fname) == True):
                # read in hic matrix
                hic_filename =  config["PROCESSED_HIC_DATA_DIR"] + 'hic_' + 'chr' + str(chr1) + '_chr' + str(chr2) + '_zscore.txt'
...
def run_LAS(pair, threshold_new):
    ...
    if (os.path.isfile(fname) == False):
        ...
        # List adhesome loci on chromosome chr2
        adhesome_loci = pd.read_csv(config['DIR_ADHESOME']+'adhesome_loci.csv', header=0, index_col=0)
        adhesome_loci_chr2 = adhesome_loci[adhesome_loci['chrom_int']==chr2]['loci'].values

        # run LAS algorithm
        start_rows, stop_rows, start_cols, stop_cols, best_score_list, avg_list = large_average_submatrix_adj(data, chr1, chr2, adhesome_loci_chr2, threshold_new)
